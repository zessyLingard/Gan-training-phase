...
def generate_cover_traffic(num_samples, seq_len):
    ...
    normal_data = np.random.normal(0.8, 0.3, (n_normal, seq_len, 1))
...
c_scaled_0 = torch.full((N_VAL, SEQ_LEN, 1), BIT0_SCALED, device=device)
c_scaled_1 = torch.full((N_VAL, SEQ_LEN, 1), BIT1_SCALED, device=device)

with torch.no_grad():
    _, expected_delta_0 = generator(z_val, c_scaled_0)
    _, expected_delta_1 = generator(z_val, c_scaled_1)

# 2. Quy đổi dự đoán về số giây vật lý
expected_ipd_0 = inverse_scale(BIT0_SCALED + expected_delta_0.cpu().numpy())
expected_ipd_1 = inverse_scale(BIT1_SCALED + expected_delta_1.cpu().numpy())

# 3. Tính khoảng cách từ thực tế đến 2 dự đoán
dist_0 = np.abs(final_val_phys - expected_ipd_0)
dist_1 = np.abs(final_val_phys - expected_ipd_1)

# 4. Giải mã: Khoảng cách nào nhỏ hơn thì chốt Bit đó!
...
decoding_errors = total_bits - correct_bits  # Số bit bị giải mã sai
reliability = (correct_bits / total_bits) * 100.0

# Vẫn giữ kiểm tra Lower Bound để đảm bảo không dính gói tin trên mạng vật lý
violations_lower = np.sum(final_val_phys.flatten() < LOWER_BOUND_PHYS)

# ── Kiểm tra Stealth (KS Test) ──
...
║                  VALIDATION RESULTS                      ║
╠══════════════════════════════════════════════════════════╣
║  Bit 0 (Lõi {BIT0_TIME}s): Mean = {bit0_ipds.mean():.4f}s   Std = {bit0_ipds.std():.4f}s       ║
║  Bit 1 (Lõi {BIT1_TIME}s): Mean = {bit1_ipds.mean():.4f}s   Std = {bit1_ipds.std():.4f}s       ║
...
║    Lower violations (< {LOWER_BOUND_PHYS}s): {violations_lower}/{len(all_ipds)} (Cảnh báo dính gói)║
...
final_val_scaled_np = final_val_scaled.cpu().numpy()
cover_sample = cover_data_scaled[:N_VAL]
...
if decoding_errors == 0 and ks_stat < 0.05:
    ...
    print("  → Suggestion: Tăng ALPHA để AI trải nhiễu đều hơn.")
elif ks_stat < 0.05:
...
ax.plot(x_range, scipy.stats.norm.pdf(x_range, 0.8, 0.15), 'r-', lw=2, label='Target N(0.8, 0.15)')
